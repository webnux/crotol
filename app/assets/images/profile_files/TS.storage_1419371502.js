(function(){TS.registerModule("storage",{msgs_version:"v1-dog",version:"0.81",prefix:window.boot_data.user_id+"_",disabled:false,buffer:{},disable_interval_buffer_write:(function(){var d,g,c,f,e,b;
b=true;
f="slack_ssb/";
e=0.45;
...
try{a.setItem(f,c)
}catch(e){TS.warn("flushBuffer _ls.setItem failed once, flushing. TS.storage.storageSize():"+TS.storage.storageSize(true));
TS.dir(0,e);
a.clear();
try{a.setItem(f,c)
...
try{a.setItem(f,c)
}catch(e){TS.warn("_set _ls.setItem failed, flushing. TS.storage.storageSize():"+TS.storage.storageSize(true));
g=true;
a.clear()
...
for(var d=0;
d<e.length;
d++){if(TS.qs_args.not_all_ls_msgs){if(d<5){continue
}}c=e[d];
if(!c.ts){continue
