(function(){TS.registerModule("api",{pending:0,limit:4,Q:[],one_at_a_time_methodsA:["users.prefs.set"],one_at_a_time_call_pending:false,one_at_a_timeQ:[],onStart:function(){},call:function(e,b,d,c){b=b||{};
var a;
...
}}TS.api.ajax_call(b,g,c,function(l){var i=(new Date().getTime()-f);
...
}else{if(l.ok===false){j=false;
if(l.error=="file_deleted"){}else{TS.error('api call "'+g+'" not ok');
try{TS.warn("args: "+JSON.stringify(c))
}catch(k){TS.warn("could not stringify args")
}try{TS.warn("data: "+JSON.stringify(l))
}catch(k){TS.warn("could not stringify data")
}}}else{TS.error('api call "'+g+'" not ok');
try{TS.warn("args: "+JSON.stringify(c))
}catch(k){TS.warn("could not stringify args")
}try{TS.warn("data: "+JSON.stringify(l))
}catch(k){TS.warn("could not stringify data")
}if(c._attempts<3){var h=(TS.api.one_at_a_time_methodsA.indexOf(g)!=-1)?TS.api.one_at_a_timeQ:TS.api.Q;
...
}else{j({ok:0,error:"Non-200 HTTP status: "+e.status,debug:e.responseText});
if(e.status===0){i=3000
...
}if(!i.is_member&&!i.is_archived){if(g){TS.error("I never ever expect to get here, but I am logging this just in case!")
}else{TS.model.requested_channel_joins[h]={and_send_txt:f};
TS.channels.join(i.name)
}return
}var e=(g)?false:j;
if(TS.client.channelDisplaySwitched(h,null,null,g,e)){TS.channels.switched_sig.dispatch()
}if(f){TS.channels.sendMsg(h,$.trim(f))
...
}if(!i.needs_created_message){i.needs_joined_message=true
...
var h;
if(TS.boot_data.feature_channel_eventlog_client){if("latest_visible" in m){m.latest=m.latest_visible
}if("unread_count_visible" in m){m.unread_count=m.unread_count_visible
...
if(m.is_member){TS.shared.checkInitialMsgHistory(m,TS.channels)
...
return true
},maybeLoadScrollBackHistory:function(f){var e=TS.channels.getChannelById(f);
if(!e){return false
...
},onHistory:function(f,i,e){var h=TS.channels.getChannelById(e.channel);
if(!h){TS.error('wtf no channel "'+e.channel+'"');
return
}if(!f||!i||!i.messages){if(TS.model.team.domain==="tinyspeck"){var g='Received no "'+h.name+'" channel history data from a call to channels.history. LocalStorage is probably incorrect for this user.';
if(i){g+="\ndata: "+JSON.stringify(i,null,"  ")
}if(e){g+="\nargs: "+JSON.stringify(e,null,"  ")
}TS.logError({message:"Failed to get channel history"},g);
if(!h._ts_alerted_on_failed_history){h._ts_alerted_on_failed_history=true;
setTimeout(function(){TS.generic_dialog.start({title:"Tiny Speck team only alert!",body:"<p>An error condition has been detected, and you should say something about it in the #dhtml channel.</p>"+g.replace(/\n/g,"<br>"),show_cancel_button:false,esc_for_ok:true,on_go:function(){}})
},2000)
}e._total_attempts=(!e._total_attempts)?e._attempts:e._total_attempts+e._attempts;
e._attempts=0;
e._retry_delay=(!e._retry_delay)?1000*2:Math.min(e._retry_delay*2,1000*60);
if(e._total_attempts>24){h.history_is_being_fetched=false;
setTimeout(function(){TS.generic_dialog.start({title:"Tiny Speck team only alert!",body:"<p>We tried to recover from the last error but failed; please post this in the #dhtml channel as well.</p>"+g.replace(/\n/g,"<br>"),show_cancel_button:false,esc_for_ok:true,on_go:function(){}})
},0);
return
}setTimeout(function(){TS.api.call("channels.history",e,TS.channels.onHistory)
},e._retry_delay)
}else{TS.error("failed to get history");
h.history_is_being_fetched=false;
TS.channels.history_fetched_sig.dispatch(h)
}return
}var k=TS.shared.onHistory(h,i,e,TS.channels);
if(!k){h.history_is_being_fetched=false;
TS.channels.history_fetched_sig.dispatch(h)
}var j=TS.utility.msgs.shouldMarkUnreadsOnMessageFetch();
TS.channels.calcUnreadCnts(h,j)
},fetchHistory:function(f,e){if(!f){TS.error('wtf no channel "'+f+'"');
return
...
if(TS.client.channelDisplaySwitched(null,null,i,h,f)){TS.groups.switched_sig.dispatch()
...
var j;
if(TS.boot_data.feature_channel_eventlog_client){if("latest_visible" in o){o.latest=o.latest_visible
}if("unread_count_visible" in o){o.unread_count=o.unread_count_visible
...
if(o.is_open||o.unread_cnt){TS.shared.checkInitialMsgHistory(o,TS.groups)
...
return true
},maybeLoadScrollBackHistory:function(g){var f=TS.groups.getGroupById(g);
if(!f){return false
...
},fetchHistory:function(g,f){if(!g){TS.error('wtf no group "'+g+'"');
return
...
(function(){TS.registerModule("files",{member_files_fetched_sig:new signals.Signal(),team_files_fetched_sig:new signals.Signal(),team_file_added_sig:new signals.Signal(),team_file_deleted_sig:new signals.Signal(),team_file_changed_sig:new signals.Signal(),team_file_comment_added_sig:new signals.Signal(),team_file_comment_edited_sig:new signals.Signal(),team_file_comment_deleted_sig:new signals.Signal(),file_uploaded_sig:new signals.Signal(),file_uploading_sig:new signals.Signal(),file_progress_sig:new signals.Signal(),file_canceled_sig:new signals.Signal(),file_queue_emptied_sig:new signals.Signal(),uploadQ:[],uploading:false,polling_count:0,polling_file_id:null,polling_ticket:null,polling_tim:null,polling_handler:null,waiting_for_refresh:{},onStart:function(){},promptForFileUnshare:function(e,c){var d=TS.channels.getChannelById(c);
var f=(d)?null:TS.groups.getGroupById(c);
if(!f&&!d){return
}TS.generic_dialog.start({title:"Un-share file",body:"<p>Are you sure you want to un-share this file from the <b>"+((d)?"#"+d.name+"</b> channel":f.name+"</b> group")+"?</p>					<p>Un-sharing the file will not remove existing share and comment messages, but it will keep any future comments from appearing 					in the "+((d)?"channel":"group")+".</p>",show_cancel_button:true,show_go_button:true,go_button_text:"Yes, unshare this file",cancel_button_text:"Cancel",on_go:function(){TS.files.unshareFile(e,c)
}})
},shareFile:function(f,c,e,d){TS.api.call("files.share",{file:f,channel:c,comment:e||""},function(h,i,g){TS.files.onFileShare(h,i,g);
if(d){d(h,i,g)
}})
},onFileShare:function(d,e,c){if(!d){return
}if(TS.web){TS.files.fetchFileInfo(c.file,function(g,f){TS.files.upsertAndSignal(f)
})
}},unshareFile:function(e,c,d){TS.api.call("files.unshare",{file:e,channel:c},function(g,h,f){TS.files.onFileUnShare(g,h,f);
if(d){d(e,TS.files.getFileById(e))
}})
},onFileUnShare:function(d,e,c){if(!d){return
}else{if(TS.web){TS.files.fetchFileInfo(c.file,function(g,f){TS.files.upsertAndSignal(f)
})
}}},fetchFileInfo:function(d,c){TS.api.call("files.info",{file:d},function(f,g,e){TS.files.onFileFetch(f,g,e);
if(c){c(d,TS.files.getFileById(d))
}})
},onFileFetch:function(d,f,c){if(!d){if(f.error=="file_deleted"){var e=TS.files.getFileById(c.file);
if(e){TS.files.removeFile(e.id);
TS.activity.maybeUpdateTeamActivity()
}else{if(c.file){TS.files.removeFile(c.file)
}}}else{if(f.error=="file_not_found"){TS.files.removeFile(c.file)
}}return
}if(f.file){f.file.comments=f.comments;
...
}},fetchTeamFiles:function(d){d=TS.model.file_list_types;
var c=(d&&d.length)?d.join(","):"";
TS.api.call("files.list",{types:c},TS.files.onTeamFetch)
},onTeamFetch:function(f,g,c){if(!f){return
}if(g.files){var e;
for(var d=0;
d<g.files.length;
d++){e=g.files[d];
...
},fetchMemberFiles:function(e,d){var c=(d&&d.length)?d.join(","):"";
TS.api.call("files.list",{user:e,types:c},TS.files.onMemberFetch)
},onMemberFetch:function(f,g,c){if(!f){return
}if(g.files){var e;
for(var d=0;
d<g.files.length;
d++){e=g.files[d];
...
},addComment:function(e,d,c){TS.api.callImmediately("files.comments.add",{file:e,comment:d},function(g,h,f){TS.files.onFileComment(g,h,f);
if(c){c(g,h,f)
}})
},onFileComment:function(e,f,c){if(!e){return
}var d=TS.files.getFileById(c.file);
if(!d){TS.error("no file? "+c.file);
return
}TS.files.addCommentToFile(f.comment,d)
},getFileById:function(f){var e=TS.model.files;
var d;
for(var c=0;
c<e.length;
c++){d=e[c];
...
},getFileActions:function(c){if(!c){return
}var d={};
var e=false;
if(c.user==TS.model.user.id){e=true
}if(c.is_public){d.share=true
}else{if(e){d.share=true
}}d.comment=true;
if(!c.public_url_shared&&c.mode!="external"&&!TS.model.user.is_restricted){if(c.is_public){d.create_public_link=true
}else{if(e){d.create_public_link=true
}}}if(c.public_url_shared&&!TS.model.user.is_restricted&&(TS.model.user.is_admin||e)){d.revoke_public_link=true
}if(c.mode=="hosted"||c.mode=="snippet"){d.download=true
}if((c.mimetype&&c.mimetype.indexOf("image/")===0)||c.mode=="external"||c.mode=="snippet"){d.open_original=true
}if(TS.web){if(c.mode=="post"||c.mode=="snippet"){d.print=true
}}if(e){if(c.mode=="snippet"||c.mode=="post"){d.edit=true
}if(c.mode=="hosted"){d.edit_title=true
}d.delete_file=true
}if(TS.model.user.is_admin){d.delete_file=true
}if(c.mode=="external"){if(e||TS.model.user.is_admin){d.refresh=true
}}if(window.Dropbox&&Dropbox.isBrowserSupported()&&TS.model.prefs.dropbox_enabled){if(c.mode=="hosted"){d.save_to_dropbox=true
}}return d
},sortFiles:function(c){function d(f,e){if(f.timestamp<e.timestamp){return 1
}if(f.timestamp>e.timestamp){return -1
}return 0
}c.sort(d)
},getFileCommentById:function(d,e){var f;
for(var c=0;
c<d.comments.length;
c++){f=d.comments[c];
if(f.id==e){return f
}}return null
},addCommentToFile:function(e,c){var d=TS.files.getFileCommentById(c,e.id);
if(d){return d
}c.comments.push(e);
TS.files.sortCommentsOnFile(c);
TS.files.team_file_comment_added_sig.dispatch(c,e);
return e
},editCommentOnFile:function(h,d){var g;
var f=false;
var e=false;
for(var c=0;
c<d.comments.length;
...
if(g.id==h.id){f=true;
d.comments[c]=h;
if(g.is_starred){h.is_starred=true
}if(d.initial_comment&&g.id==d.initial_comment.id){d.initial_comment=h;
e=true
}break
}}if(!f){return
...
if(e){TS.files.team_file_changed_sig.dispatch(d)
}},deleteCommentOnFile:function(f,e){var h;
var c=[];
var g=false;
for(var d=0;
d<e.comments.length;
d++){h=e.comments[d];
if(h.id==f){if(e.initial_comment&&h.id==e.initial_comment.id){e.initial_comment=null;
g=true
}continue
}c.push(h)
}if(c.length==e.comments.length){return
}e.comments=c;
...
if(g){TS.files.team_file_changed_sig.dispatch(e)
}},makeSureReferencesGetSavedToLS:function(g){var d=function(l,k){var m;
for(var j=0;
j<l.length;
j++){m=l[j];
if(m.file&&m.file.id==k){return true
}}return false
};
var e;
var f;
...
e<TS.model.channels.length;
e++){f=TS.model.channels[e];
if(f&&f.msgs&&f.msgs.length){if(d(f.msgs,g)){TS.utility.msgs.maybeStoreMsgs(f.id,f.msgs)
}}}var h;
for(e=0;
e<TS.model.groups.length;
e++){h=TS.model.groups[e];
if(h&&h.msgs&&h.msgs.length){if(d(h.msgs,g)){TS.utility.msgs.maybeStoreMsgs(h.id,h.msgs)
}}}var c;
for(e=0;
e<TS.model.ims.length;
e++){c=TS.model.ims[e];
if(c&&c.msgs&&c.msgs.length){if(d(c.msgs,g)){TS.utility.msgs.maybeStoreMsgs(c.id,c.msgs)
}}}},sortCommentsOnFile:function(c){function d(f,e){if(f.timestamp>e.timestamp){return 1
}if(f.timestamp<e.timestamp){return -1
}return 0
}c.comments.sort(d)
},upsertFile:function(e){var c=TS.model.files;
...
d=f.ims||[];
n=f.groups||[];
for(var g in e){if(g=="channels"){if(e.channels&&f.channels.join("")!=e.channels.join("")){if(e.channels&&e.channels.length){s=s.concat(e.channels)
}f.channels=e.channels;
h="CHANGED";
p.push(g)
}}else{if(g=="ims"){if(e.ims&&f.ims.join("")!=e.ims.join("")){if(e.ims&&e.ims.length){d=d.concat(e.ims)
}f.ims=e.ims;
h="CHANGED";
p.push(g)
}}else{if(g=="groups"){if(e.groups&&f.groups.join("")!=e.groups.join("")){if(e.groups&&e.groups.length){n=n.concat(e.groups)
}f.groups=e.groups;
h="CHANGED";
p.push(g)
}}else{if(g=="comments"){if(e.comments&&JSON.stringify(f.comments)!=JSON.stringify(e.comments)){f.comments=e.comments;
h="CHANGED";
p.push(g)
}}else{if(g=="content"){if(e.content&&f.content!=e.content){f.content=e.content;
h="CHANGED";
p.push(g)
}}else{if(g=="initial_comment"){f[g]=e[g]
}else{if(f[g]!=e[g]){if(e[g]&&!TS.utility.isScalar(e[g])){f[g]=e[g];
TS.warn(g+" is not scalar! it needs to be handled by upsertFile specifically to test if it has changed! "+(typeof e[g]))
}else{if(typeof e[g]!="boolean"||!e[g]!=!f[g]){p.push(g+" ["+f[g]+"] -> ["+e[g]+"]");
f[g]=e[g];
h="CHANGED"
}}}}}}}}}}}else{h="ADDED";
c.push(e);
...
m.has_files=true
}else{TS.error("hmmm, file "+e.id+" does not have a know user "+e.user)
}a(e);
f=e
}if(h=="CHANGED"){var l;
if(e.thumb_720&&TS.utility.isRetina()){l=e.thumb_360_gif||e.thumb_720
}else{l=e.thumb_360_gif||e.thumb_360
}if(l&&!TS.model.inline_imgs[l]){a(e)
}}if(!f.comments){f.comments=[]
}else{f.comments_count=Math.max(f.comments_count,f.comments.length)
}if(!f.channels){f.channels=[]
}if(!f.ims){f.ims=[]
}if(!f.groups){f.groups=[]
}f.is_shared=(f.groups.length>0)||(f.channels.length>0);
if(h!="NOOP"){var j;
var q;
if(s){s=s.filter(function(u,t,k){return k.indexOf(u)===t
});
var o;
for(j=0;
j<s.length;
j++){o=TS.channels.getChannelById(s[j]);
if(o&&o.is_member&&o.msgs&&o.msgs.length){TS.utility.msgs.maybeStoreMsgs(o.id,o.msgs)
}}}if(d){d=d.filter(function(u,t,k){return k.indexOf(u)===t
});
for(j=0;
j<d.length;
j++){q=TS.ims.getImById(d[j]);
if(q&&q.msgs&&q.msgs.length){TS.utility.msgs.maybeStoreMsgs(q.id,q.msgs)
}}}if(n){n=n.filter(function(u,t,k){return k.indexOf(u)===t
});
var r;
for(j=0;
j<n.length;
j++){r=TS.groups.getGroupById(n[j]);
if(r&&r.msgs&&r.msgs.length){TS.utility.msgs.maybeStoreMsgs(r.id,r.msgs)
}}}if(!d||!d.length){q=TS.ims.getImByMemberId(TS.model.user.id);
if(q&&q.msgs&&q.msgs.length){TS.utility.msgs.maybeStoreMsgs(q.id,q.msgs)
}if(f.user!=TS.model.user.id){q=TS.ims.getImByMemberId(f.user);
if(q&&q.msgs&&q.msgs.length){TS.utility.msgs.maybeStoreMsgs(q.id,q.msgs)
}}}}TS.files.sortFiles(TS.model.files);
return{status:h,file:f,what_changed:p}
},upsertAndSignal:function(c){var d=TS.files.upsertFile(c);
if(d.status=="CHANGED"){TS.files.team_file_changed_sig.dispatch(d.file)
}else{if(d.status=="ADDED"){TS.files.team_file_added_sig.dispatch(d.file)
}}return d
},removeFile:function(g){TS.log(4,'removing file "'+g+'"');
var e;
...
e<f.length;
e++){h=f[e];
if(d){TS.utility.msgs.removeFileSharesAndMentions(h,d)
}if(d){TS.utility.msgs.removeFileComments(h,d)
...
e<c.length;
e++){k=c[e];
if(d){TS.utility.msgs.removeFileSharesAndMentions(k,d)
}if(d){TS.utility.msgs.removeFileComments(k,d)
...
e<l.length;
e++){j=l[e];
if(d){TS.utility.msgs.removeFileSharesAndMentions(j,d)
}if(d){TS.utility.msgs.removeFileComments(j,d)
}TS.utility.msgs.removeFileReferences(j,g)
}if(d){TS.files.team_file_deleted_sig.dispatch(d)
}},upload:function(k,e,i,c,j,h,f,g,d){if(TS.files.uploading){TS.files.uploadQ.push(arguments)
}else{TS.files.actuallyUpload(k,e,i,c,j,h,f,g,d)
}},actuallyUpload:function(q,h,m,c,n,l,i,k,g){TS.files.uploading=true;
g=g||0;
var j=new FormData();
var p;
var e=!!h;
if(q){p=n||l;
TS.files.file_uploading_sig.dispatch(p,g>0,e);
j.append("content",q);
if(l){j.append("filetype",l)
}if(c){TS.warn("ignoring filename because it makes no sense for text files")
}}else{p=n||c||h.name||"blob";
TS.files.file_uploading_sig.dispatch(p,g>0,e);
if(typeof h=="string"){j.append("content64",h)
}else{j.append("file",h)
}if(c){j.append("filename",c)
}if(l){TS.warn("ignoring filetype we send a filename which can intuit it")
}}j.append("token",TS.model.api_token);
if(i&&i.length){var o=(typeof i=="string")?i:(i.join)?i.join(","):"";
j.append("channels",o)
}j.append("title",n);
if(k){j.append("initial_comment",k)
}var r="files.uploadAsync";
if(m){r="services.dropbox.upload";
j.append("link",m)
}TS.log(2,"calling "+TS.model.api_url+"files.upload");
var d;
if(r=="files.uploadAsync"){d=TS.model.async_api_url+r
}else{d=TS.model.api_url+r
}var f=false;
b=$.ajax({url:d,data:j,dataType:"json",cache:false,contentType:false,processData:false,type:"POST",xhr:function(){var s=jQuery.ajaxSettings.xhr();
if(s.upload){s.upload.addEventListener("progress",function(t){if(t.lengthComputable){var u=parseInt(100*t.loaded/t.total,10);
TS.files.file_progress_sig.dispatch(u)
}else{TS.info("Upload length not computable")
}},false)
}return s
},error:function(s,u,t){f=true;
TS.info("Error: Failed to upload file.");
TS.info("textStatus:"+u+" errorThrown:"+t);
if(u==="abort"){TS.files.file_canceled_sig.dispatch(p);
TS.files.uploadOver(false);
return
}if(g===0){TS.files.actuallyUpload(q,h,m,c,n,l,i,k,++g)
}else{TS.generic_dialog.start({title:"Upload failed",body:'Failed to upload file: "'+u+(t?" "+t:"")+'". Try again?',go_button_text:"Yes, try again",cancel_button_text:"No, cancel",on_go:function(){TS.files.actuallyUpload(q,h,m,c,n,l,i,k,++g)
},on_cancel:function(){TS.files.uploadOver(false)
}})
}},complete:function(t){if(f){return
}t=jQuery.parseJSON(t.responseText);
if(t&&t.ok&&t.file){if(r=="files.uploadAsync"){var u=function(x,z,w){if(!TS.files.polling_file_id){return
}if(x){if(z.status=="complete"){var y=TS.files.upsertAndSignal(z.file);
TS.files.uploadProcessingOver(true,y.file.id)
}else{if(z.status=="failed"){var v="";
if(z.debug&&TS.model.team.domain=="tinyspeck"){v="<br><br>TS only Debugging:<br><br>"+z.debug
}TS.generic_dialog.start({title:"Upload failed",body:"Failed to process the uploaded file. Try again?"+v,go_button_text:"Yes, try again",cancel_button_text:"No, cancel",on_go:function(){TS.files.actuallyUpload(q,h,m,c,n,l,i,k,++g)
...
TS.files.startPollingForUploadProcessing(t.file,t.ticket,u)
}else{var s=TS.files.upsertAndSignal(t.file);
TS.files.uploadOver(t.ok,s.file.id)
}}else{TS.info("Error: Failed to upload file.");
TS.info(t);
if(t){if(g===0){TS.files.actuallyUpload(q,h,m,c,n,l,i,k,++g)
}else{TS.generic_dialog.start({title:"Upload failed",body:'Failed to upload file: "'+(t.error||"unknown error")+'". Try again?',go_button_text:"Yes, try again",cancel_button_text:"No, cancel",on_go:function(){TS.files.actuallyUpload(q,h,m,c,n,l,i,k,++g)
},on_cancel:function(){TS.files.uploadOver(false)
}})
}}else{alert("Upload failed.");
TS.files.uploadOver(false)
}}}})
},startPollingForUploadProcessing:function(c,d,e){TS.files.polling_count=0;
...
}TS.api.callImmediately("files.uploadStatus",{ticket:TS.files.polling_ticket},function(d,e,c){if(!TS.files.polling_ticket){return
}TS.files.polling_handler(d,e,c)
})
},TS.files.polling_count*1000)
},uploadProcessingOver:function(c,d){if(TS.files.polling_file_id!=d){return
...
},uploadOver:function(c,d){TS.files.file_uploaded_sig.dispatch(c,d);
...
}},deleteFile:function(c){TS.api.call("files.delete",{file:c},TS.files.onFileDelete)
},onFileDelete:function(d,e,c){if(!d){return
}},endEditFileTitle:function(){$("#file_edit_title_container").addClass("hidden");
$("#file_title_container").removeClass("hidden")
},saveEditFileTitle:function(e){var d=TS.files.getFileById(e);
if(!d){return
}var f=$("#file_edit_title_input").val();
if(!$.trim(f)){TS.sounds.play("beep");
return
}var c=d.title;
if(c==f){TS.files.endEditFileTitle();
return
}TS.api.callImmediately("files.edit",{file:e,title:f},function(h,i,g){if(!h){TS.files.upsertAndSignal({id:e,title:c});
alert("save failed!")
}});
f=TS.utility.htmlEntities(f);
TS.files.upsertAndSignal({id:e,title:f});
TS.files.endEditFileTitle()
},editFileTitle:function(d){var c=TS.files.getFileById(d);
if(!c){return
}var e=c.title;
if(e){e=TS.format.unFormatMsg(e)
}else{e=c.name
...
},onDropboxChooser:function(f){TS.dir(2,f);
var e=[];
for(var c=0;
c<f.length;
c++){var g=f[c];
e.push({name:g.name,size:g.bytes,link:g.link,icon:g.icon,is_dropbox:true})
}TS.ui.upload_dialog.startWithCommentFromChatInput(e)
},makeFileNameFromFile:function(d){var c=TS.utility.date.getTimeStamp()/1000;
return d.name||"Pasted image at "+TS.utility.date.toFilenameFriendlyDate(c)+".png"
},makeFileTitleFromFile:function(d){var c=TS.utility.date.getTimeStamp()/1000;
return d.name||"Pasted image at "+TS.utility.date.toDate(c)
},justUploadTheseFileNow:function(e){var d;
for(var c=0;
c<e.length;
c++){d=e[c];
if(d.size>TS.model.upload_file_size_limit_bytes){continue
...
}},refreshFile:function(c){TS.files.startRefreshingFile(c);
TS.api.call("files.refresh",{file:c},TS.files.onFileRefresh)
},onFileRefresh:function(d,f,c){var g=c.file;
if(d){TS.menu.$menu.find("#refresh_file").find(".item_label").text("File refreshed!").end().find("i").removeClass("fa-spin")
}else{if(!d){TS.files.doneRefreshingFile(g,'<span class="moscow_red">Refresh failed.</span>',5000);
TS.menu.$menu.find("#refresh_file").find(".item_label").text("Refresh failed").end().find("i").removeClass("fa-spin")
}}if(d&!f.will_refresh){TS.files.doneRefreshingFile(g,'<span class="moscow_red">File refreshed < 1 minute ago.</span>',5000)
}if(TS.web&&d){TS.menu.$menu.find("#refresh_file").find(".item_label").text("Reloading...");
location.reload()
}if(!d){if(f.error=="file_deleted"){var e=TS.files.getFileById(g);
if(e){TS.files.removeFile(e.id);
TS.activity.maybeUpdateTeamActivity()
}}return
}},fileWasMaybeRefreshed:function(c){if(!c){return
}if(!TS.files.waiting_for_refresh[c.id]){return
}TS.files.doneRefreshingFile(c.id,'<span class="kelly_green">File refreshed!</span>',60000)
},startRefreshingFile:function(c){TS.files.waiting_for_refresh[c]=true;
$('.file_refresh[data-file-id="'+c+'"]').addClass("hidden");
$('.file_refresh_status[data-file-id="'+c+'"]').removeClass("hidden")
},doneRefreshingFile:function(e,d,c){delete TS.files.waiting_for_refresh[e];
$('.file_refresh_status[data-file-id="'+e+'"]').html(d);
setTimeout(function(){$('.file_refresh[data-file-id="'+e+'"]').removeClass("hidden");
$('.file_refresh_status[data-file-id="'+e+'"]').text("Refreshing file...").addClass("hidden")
},c)
}});
var b=null;
var a=function(d){var c;
if(TS.utility.isRetina()&&d.thumb_720){c=d.thumb_360_gif||d.thumb_720
}else{c=d.thumb_360_gif||d.thumb_360
}if(c&&TS.model.inline_img_exclude_filetypes.indexOf(d.filetype)==-1){TS.inline_imgs.makeInternalInlineImg(c,{width:d.thumb_360_w,height:d.thumb_360_h,link_url:d.url_private,internal_file_id:d.id})
}}
})();
...
if(TS.client.channelDisplaySwitched(null,h,null,false,d)){TS.ims.switched_sig.dispatch()
...
if(TS.boot_data.feature_channel_eventlog_client){if("latest_visible" in f){f.latest=f.latest_visible
}if("unread_count_visible" in f){f.unread_count=f.unread_count_visible
}}if(i){TS.log(4,'updating existing im "'+f.id+'"');
for(var g in f){i[g]=f[g]
}f=i;
if(f.is_open||f.unread_cnt){TS.shared.checkInitialMsgHistory(f,TS.ims)
...
return true
},maybeLoadScrollBackHistory:function(e){var d=TS.ims.getImById(e);
if(!d){return false
...
}}},fetchHistory:function(d,e){if(!d){TS.error('wtf no im "'+d+'"');
return
...
if(m>h){n.push({im:o,ms_since_activity:m})
...
(function(){TS.registerModule("shared",{onStart:function(){},calcUnreadCnts:function(a,e,n){a.unreads.length=0;
...
}}},maybeLoadScrollBackHistory:function(b,a){if(b.scroll_top!==0){return false
...
},sendMsg:function(c,e,a){if(!e){return false
}var d=TS.utility.date.makeTsStamp();
e=TS.format.cleanMsg(e);
if(e.indexOf("DELEEEEETETEEESTTTT")===0){TS.ms.disconnect()
}var b=TS.ms.send({type:"message",channel:c,text:$.trim(e)},a.onSendMsg,d);
TS.typing.userEnded(TS.ims.getImById(c)||TS.groups.getGroupById(c)||TS.channels.getChannelById(c));
a.addMsg(c,{type:"message",text:e,user:TS.model.user.id,ts:d,rsp_id:b});
return true
...
}}if(s.is_self&&s.deleted){TS.info("calling TS.client.reload() because member.is_self && member.deleted");
...
},filterTeam:function(h,e,b){var s=$(e);
a=h;
var d=s.data("list-items-id");
var i=$(d);
var q=i.find(".member_item");
var t={};
q.removeClass("active");
i.find(".no_results").addClass("hidden");
function j(w,v){var u=$.grep(w,function(z,y){var x=(z.name&&z.name.match(v))||(z.first_name&&z.first_name.match(v))||(z.last_name&&z.last_name.match(v))||(z._real_name_lc&&z._real_name_lc.match(v))||(z.profile&&z.profile.email&&z.profile.email.match(v))||(z.profile&&z.profile.real_name_normalized&&z.profile.real_name_normalized.match(v))||(z.profile&&z.profile.real_name&&z.profile.real_name.match(v));
return x
});
return u
}function n(x,w){var v=new RegExp("^"+TS.utility.regexpEscape(w),"i");
var z=new RegExp("(-|_|\\+|\\s|\\.|@)"+TS.utility.regexpEscape(w),"i");
var y=j(x,v);
var u=j(x,z);
u=u.filter(function(A){return !y.some(function(B){return B.id===A.id
})
});
return y.concat(u)
}var o;
if(TS.web&&TS.web.admin&&TS.web.admin.view=="invites"){var r=[];
var g=[];
var p=[];
$.each(q,function(u,v){var w;
v=$(v);
w=v.data("invite-id");
t[w]=v
});
g=n(TS.web.admin.pending_invites,h);
p=n(TS.web.admin.accepted_invites,h);
r=g.concat(p);
if(r.length>0){$.each(r,function(v,u){if(u&&u.id&&t[u.id]){t[u.id].addClass("active")
}});
if(b){$(b).trigger("resize")
}}o=[{name:"pending",label:"pending invitations",matches:g},{name:"accepted",label:"accepted invitations",matches:p}];
o.forEach(function(w){var u=$("#"+w.name+"_no_results");
if(w.matches.length>0){u.addClass("hidden").empty();
return
}else{var x={query:h,tab:w,pending_matches:g,show_pending_matches:w.name!="pending"&&g.length>0,accepted_matches:p,show_accepted_matches:w.name!="accepted"&&p.length>0};
var v=TS.templates.team_list_no_results(x);
u.removeClass("hidden").html(v);
u.find(".clear_members_filter").on("click",function(){TS.members.view.clearFilter(e,b)
})
}})
}else{var l=TS.members.getMembersForUser();
var m=[];
var f=[];
var k=[];
...
var c=n(l,h);
if(c.length>0){$.each(c,function(u,v){if(v&&v.id&&t[v.id]){t[v.id].addClass("active")
}if(v.is_restricted){f.push(v)
}else{if(v.deleted){k.push(v)
}else{m.push(v)
}}});
if(b){$(b).trigger("resize")
}}else{if(e=="#dms_filter"||e=="#file_member_filter"){i.find(".query").text(h);
i.find(".no_results").removeClass("hidden")
}}if(e=="#team_filter"){o=[{name:"active",label:"full team members",matches:m},{name:"restricted",label:"restricted accounts",matches:f},{name:"disabled",label:"disabled accounts",matches:k}];
o.forEach(function(w){var u=$("#"+w.name+"_no_results");
if(w.matches.length>0){u.addClass("hidden").empty();
return
}else{var x={query:h,tab:w,active_matches:m,show_active_matches:w.name!="active"&&m.length>0,restricted_matches:f,show_restricted_matches:w.name!="restricted"&&f.length>0,disabled_matches:k,show_disabled_matches:TS.web&&w.name!="disabled"&&k.length>0};
var v=TS.templates.team_list_no_results(x);
u.removeClass("hidden").html(v);
u.find(".clear_members_filter").on("click",function(){TS.members.view.clearFilter(e,b)
})
...
}},onTeamDirectoryItemClick:function(b){if($(b.target).closest("a").length){return
...
var b;
var a;
if(TS.model.muted_channels){for(b=0;
...
}}else{if(a.name=="compliance_export_start"){if(TS.model.prefs.compliance_export_start!=a.value){TS.model.prefs.compliance_export_start=a.value;
...
if(TS.model.mac_ssb_version>=0.44){emoji.sheet_path="slack-resources:emoji_hangouts_64_indexed_256colors.png"
...
if(TS.model.mac_ssb_version>=0.64){emoji.sheet_path="slack-resources:emoji_emojione_64_indexed_256colors.png"
...
if(TS.model.mac_ssb_version>=0.43){emoji.sheet_path="slack-resources:emoji_twitter_64_indexed_256colors.png"
...
if(TS.model.mac_ssb_version>=0.43){emoji.sheet_path="slack-resources:emoji_apple_64_indexed_256colors.png"
...
},dispatchSearch:function(i,g,k,h){var f=a(i,g,h);
...
}else{TS.api.call("search.all",f,k)
...
TS.ms.sent_map[K.id.toString()]={msg:K,handler:I,ts:TS.utility.date.getTimeStamp(),temp_ts:J};
if(K.type=="ping"||K.type=="pong"){TS.log(3,"sending "+K.type);
TS.dir(3,K)
}else{TS.model.last_net_send=TS.utility.date.getTimeStamp();
TS.log(2,"sending "+K.type);
TS.dir(2,K)
}s.send(JSON.stringify(K));
return K.id
},sendTyping:function(I){var J='{"type":"typing", "channel":"'+I+'"}';
s.send(J)
},handleMsg:function(I){var L=I.reply_to&&!("ok" in I)&&I.type=="message";
if(L){}var J;
if(I.reply_to){if(I.reply_to.toString() in TS.ms.sent_map){J=TS.ms.sent_map[I.reply_to];
I.SENT_MSG=J.msg;
delete TS.ms.sent_map[I.reply_to]
}else{if(!L){TS.error('received msg "'+I.reply_to+'" with type "'+I.type+'" but we have no record of it in sent_map')
}}}else{if(I.event_ts){TS.ms.storeLastEventTS(I.event_ts)
}}if(I.type=="ping"||I.type=="pong"){TS.log(3,"msg "+I.type+" time: "+(TS.utility.date.getTimeStamp()-J.ts)+"ms");
...
TS.log(2,"msg "+((K)?'"'+K+'" ':"")+"rsp time "+(TS.utility.date.getTimeStamp()-J.ts)+"ms")
}else{TS.log(2,'msg "'+I.type+'"')
}TS.dir(2,I)
}if(I.type=="error"){h(I)
}else{if(I.type=="hello"){k()
}else{if(!I.reply_to){TS.ms.on_msg_sig.dispatch(I)
}}}if(J){if(!I.ok){I.error=I.error||{code:0,msg:"unknown error (not specified by MS)"}
}if(L){I.ok=true
}if(J.handler){J.handler(I.ok,I)
}}},storeLastEventTS:function(J){if(!J){return
}var I=TS.storage.fetchLastEventTS();
if(I&&J<=I){return
}TS.storage.storeLastEventTS(J)
},onFailure:function(K){TS.info("TS.ms.onFailure reason_str:"+K);
if(K){x("You got disconnected and are on team Tiny Speck, so here are some details:\n>>>"+K)
}b=false;
d();
...
var J=TS.model.ms_reconnect_ms=((TS.model.ms_reconnect_ms+1000)*1.3);
if(TS.model.ms_reconnect_ms>4000){TS.model.ms_reconnect_ms=TS.utility.randomInt(J,J+(J/3))
}TS.model.ms_reconnect_ms=Math.min(TS.model.ms_reconnect_ms,300000)
}if(TS.model.rtm_start_throttler>5){var I=2000*TS.model.rtm_start_throttler;
if(TS.model.ms_reconnect_ms<I){TS.info("because TS.model.rtm_start_throttler:"+TS.model.rtm_start_throttler+" we are increasing time until next login call");
TS.model.ms_reconnect_ms-I
...
c();
clearTimeout(n);
n=setTimeout(function(){if(!TS.model.window_unloading){TS.ms.reconnect_requested_sig.dispatch()
...
}},disconnect:function(){if(s&&TS.model.ms_connected){TS.ms.logConnectionFlow("disconnect");
s.close()
}else{TS.warn("TS.ms.disconnect called, but _websocket="+s+" TS.model.ms_connected="+TS.model.ms_connected)
}},logConnectionFlow:function(I){var J=TS.model.ms_conn_log;
var K=TS.utility.date.getTimeStamp();
J.push({name:I,time:K,delta:(J.length)?K-J[J.length-1].time:0});
TS.log(2,"logConnectionFlow "+I+" "+J[J.length-1].delta)
},getConnectionFlowLog:function(M){var L=TS.model.ms_conn_log;
var I=[];
for(var J=0;
J<L.length;
...
var K=I.join("&");
if(M&&K.length>M){K=K.substr(0,M)
}return K
...
J=TS.model.team.url;
var M=2000-J.length;
var K=TS.ms.getConnectionFlowLog(M);
var L=(TS.qs_args.simulate_old_token==1)?"&TRIGGER_OLD_TOKEN=1":"";
J+="?svn_rev="+TS.boot_data.svn_rev+L+"&"+K;
TS.info("Full WSS url: "+J);
...
}E=(window.WEB_SOCKET_USING_FLASH)?v:D;
TS.info("_connect_timeout_tim_ms:"+E);
clearTimeout(f);
f=setTimeout(o,E);
TS.ms.last_url=J;
...
if(TS.qs_args.simulate_first_connect_timeout==1&&q<1){TS.info("simulate_first_connect_timeout url:"+J)
}else{s.onopen=z
}s.onclose=j;
s.onerror=w
...
TS.log(3,"_pong_timeout_ms set to:"+G+" has_focus:"+I)
};
...
var u=function(Q,N,P){if(!Q){TS.error("_onEventLog "+N);
if(TS.client&&N&&N.error=="timestamp_too_old"){TS.storage.cleanOutMsgStorageAndReset();
var I="TS.client.reload() after a TS.storage.cleanOutMsgStorageAndReset() because data.error: <code>timestamp_too_old</code>";
if(N.reason){I+=" data.reason: <code>"+N.reason+"</code>"
}if(P){delete P.token
}try{I+=" args: <pre>"+JSON.stringify(P,null,"\t")+"</pre>"
}catch(K){}I+="<p><b>Tell eric about this, please!</b></p>";
setTimeout(TS.client.reload,1,null,I)
}return
}if(!N.events){TS.error("_onEventLog missing events");
return
}var L=1000;
if(TS.client&&N.total>L){TS.storage.cleanOutMsgStorageAndReset();
TS.info("going to call TS.client.reload() after a TS.storage.cleanOutMsgStorageAndReset() because data.total > "+L+")");
setTimeout(TS.client.reload,1,null,"TS.client.reload() after a TS.storage.cleanOutMsgStorageAndReset() because data.total > "+L+")");
return
}var J;
var R;
for(var M=0;
M<N.events.length;
M++){R=N.events[M];
if(R.event_ts){J=R.event_ts;
delete R.event_ts
}if(R.type=="slack_broadcast"){if(R.reload){var S=C;
if(S){if(S.force_reload){if(R.force_reload){C=R
}}else{C=R
}}else{C=R
}}continue
}try{TS.ms.handleMsg(R)
}catch(O){}}if(N.has_more){if(!N.events.length){TS.error(" WTF data.events.length==0 and data.has_more:true ??????")
}else{TS.api.call("eventlog.history",{start:J},u)
}}else{if(J){TS.ms.storeLastEventTS(J)
}if(C){try{TS.ms.handleMsg(C)
}catch(O){}C=null
}}};
var y=function(){if(b){var J=TS.utility.date.getTimeStamp()-TS.ms.last_pong_time;
TS.log(3,"since_last_pong_ms:"+J+" pong_timeout_ms:"+G);
if(J>G){TS.warn("since_last_pong_ms too long! "+J+" > "+G);
...
}}TS.ms.send({type:"ping",ping_interv_ms:m,client_time:Date.now()})
};
...
clearTimeout(A);
clearTimeout(f);
if(J){TS.info("_onDisconnect event:");
TS.error(J);
...
s.onclose=null;
try{s.close()
}catch(J){}if(false){var I=s;
s.onclose=function(K){TS.ms.logConnectionFlow("old_socket_closed");
...
var c=function(){var I=TS.model.ms_reconnect_time-TS.utility.date.getTimeStamp();
var J=Math.round(I/1000);
if(J>=0){TS.ms.reconnecting_sig.dispatch(J)
...
var o=function(){q++;
var I="socket not connected "+E+"ms after creation. _connect_timeout_count:"+q;
TS.warn(I);
TS.ms.logConnectionFlow("_onConnectTimeout");
if(q==3){TS.generic_dialog.start({title:"Connection trouble",body:"<p>Apologies, we're having some trouble with your web socket connection.</p>				<p>We've seen this problem clear up with a restart of "+(TS.model.is_our_app?"Slack":"your browser")+", 				a solution which we suggest to you now only with great regret and self loathing.</p>				",show_cancel_button:false,go_button_text:"OK",esc_for_ok:true});
return
}else{if(q==2){if(window.WEB_SOCKET_USING_FLASH){}else{if(TS.model.is_chrome){window.fallBackToFlashWebSockets();
...
var h=function(I){if(I.error){if(I.error.code==1){TS.ms.logConnectionFlow("msg_error_code_1")
}else{TS.logError({message:"_onErrorMsg"},JSON.stringify(I));
TS.ms.onFailure("_onErrorMsg imsg.error:"+I.error)
}}else{TS.logError({message:"_onErrorMsg"},I?JSON.stringify(I):"no imsg?")
}};
var k=function(){clearTimeout(A);
TS.logLoad("_onHello (took "+(new Date().getTime()-TS.ms.last_start_ms)+"ms)");
var J=TS.utility.date.getTimeStamp()-TS.ms.last_pong_time;
TS.info("Hello from: "+TS.model.team.url+" since_last_pong_ms:"+J);
TS.ms.logConnectionFlow("on_hello");
if(TS.client&&J>p){TS.client.ui.maybePromptForSetActive()
}clearInterval(a);
b=true;
TS.ms.last_pong_time=TS.utility.date.getTimeStamp();
clearInterval(t);
...
var I=TS.storage.fetchLastEventTS();
if(I){TS.api.callImmediately("eventlog.history",{start:I},u)
}}
})();
(function(){TS.registerModule("ms.msg_handlers",{onStart:function(){TS.ms.on_msg_sig.add(TS.ms.msg_handlers.msgReceived)
},msgReceived:function(a){if(a.reply_to){return
}if(!TS.ms.msg_handlers[a.type]){return
}TS.ms.msg_handlers[a.type](a)
},message:function(b){TS.log(2,"recved message "+b.type);
if(b.is_ephemeral&&!b.ts){b.ts=TS.utility.date.makeTsStamp()
}var e="subtype__"+b.subtype;
if(e in TS.ms.msg_handlers){if(TS.boot_data.feature_channel_eventlog_client){if(b.subtype=="message_changed"||b.subtype=="message_deleted"||b.subtype=="channel_history_changed"||b.subtype=="group_history_changed"||b.subtype=="im_history_changed"){TS.info("converting subtype:"+b.subtype+" to type:"+b.subtype);
b.type=b.subtype;
delete b.subtype;
TS.ms.msg_handlers[b.type](b);
return
}if(b.hidden){console.error("WE SHOULD NOT BE GETTING ANY HIDDEN MESSAGES ANYMORE");
console.dir(0,b)
}}TS.ms.msg_handlers[e](b)
}var c=TS.utility.msgs.processImsg(b);
if(TS.ims.getImById(b.channel)){if(b.text=="start_profile_AAAAAA"){TS.model.profiling_keys=true
}else{if(b.text=="end_profile_AAAAAA"){TS.model.profiling_keys=false;
if(TS.model.profiling_key_times){TS.files.upload(JSON.stringify(TS.model.profiling_key_times,null,"\t"),null,null,null,"auto profile","javascript",[b.channel],"");
delete TS.model.profiling_key_times
}}}TS.ims.addMsg(b.channel,c)
}else{if(TS.groups.getGroupById(b.channel)){TS.groups.addMsg(b.channel,c)
}else{TS.channels.addMsg(b.channel,c)
...
if(d&&a){TS.typing.memberEnded(a,d)
}},subtype__file_share:function(a){if(!a.file){return
}if(a.file.id==TS.files.polling_file_id){TS.files.uploadProcessingOver(true,a.file.id)
}},message_changed:function(a){TS.log(2,"recved message "+a.type);
TS.ms.msg_handlers.message_changed_worker(a)
},subtype__message_changed:function(a){TS.log(2,"recved subtype "+a.subtype);
if(TS.boot_data.feature_channel_eventlog_client){TS.warn("feature_channel_eventlog_client=1 so we should never be getting subtype "+a.subtype)
}TS.ms.msg_handlers.message_changed_worker(a)
},message_changed_worker:function(b){if(!b.message){TS.error("no message?");
return
}TS.mentions.replaceMsg(b.message);
var c=TS.channels.getChannelById(b.channel);
var d;
var a;
if(!c){a=TS.ims.getImById(b.channel)
}if(!c&&!a){d=TS.groups.getGroupById(b.channel)
}if(!a&&!c&&!d){TS.error("unknown imsg.channel:"+b.channel);
return
}if(b.message.imgs||TS.utility.msgs.hasImgs(b.message)){TS.model.show_inline_img_size_pref_reminder=true
}TS.utility.msgs.replaceMsg(a||c||d,b.message)
},message_deleted:function(a){TS.log(2,"recved message "+a.type);
TS.ms.msg_handlers.message_deleted_worker(a)
},subtype__message_deleted:function(a){TS.log(2,"recved subtype "+a.subtype);
if(TS.boot_data.feature_channel_eventlog_client){TS.warn("feature_channel_eventlog_client=1 so we should never be getting subtype "+a.subtype)
}TS.ms.msg_handlers.message_deleted_worker(a)
},message_deleted_worker:function(d){if(!d.deleted_ts){TS.error("no deleted_ts?");
return
}TS.mentions.removeMsg(d.deleted_ts);
var e=TS.channels.getChannelById(d.channel);
var b;
var f;
if(!e){b=TS.ims.getImById(d.channel)
}if(!e&&!b){f=TS.groups.getGroupById(d.channel)
}if(!b&&!e&&!f){TS.error("unknown imsg.channel:"+d.channel);
return
}var a=b||e||f;
var c=TS.utility.msgs.getMsg(d.deleted_ts,a.msgs);
if(!c){return
}if(b){TS.ims.removeMsg(a.id,c)
}else{if(e){TS.channels.removeMsg(a.id,c)
}else{if(f){TS.groups.removeMsg(a.id,c)
}}}},channel_left:function(a){TS.info("You left channel "+a.channel);
var b=TS.channels.getChannelById(a.channel);
if(!b){TS.error('unknown channel: "'+a.channel)
}b.is_member=false;
if(TS.model.active_channel_id==a.channel&&!b.was_archived_this_session){TS.client.activeChannelDisplayGoneAway()
}TS.channels.calcUnreadCnts(b,true);
TS.members.invalidateMembersUserCanSeeArrayCaches();
TS.channels.left_sig.dispatch(b)
},subtype__channel_leave:function(b){var a=b.user;
var e=TS.members.getMemberById(a);
if(!e){TS.error('unknown member: "'+a+'"');
return
}TS.info(e.name+" left channel "+b.channel);
var d=TS.channels.getChannelById(b.channel);
if(d){for(var c=0;
c<d.members.length;
c++){if(d.members[c]==e.id){d.members.splice(c,1);
TS.channels.calcActiveMembersForChannel(d);
break
}}}TS.members.invalidateMembersUserCanSeeArrayCaches();
TS.channels.member_left_sig.dispatch(d,e)
},channel_joined:function(a){TS.info("You joined channel "+a.channel.name);
var b=TS.channels.upsertChannel(a.channel);
TS.members.invalidateMembersUserCanSeeArrayCaches();
TS.channels.joined_sig.dispatch(b)
},channel_created:function(a){TS.info("created channel "+a.channel.name);
var b=TS.channels.upsertChannel(a.channel);
TS.channels.created_sig.dispatch(b);
if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},channel_deleted:function(a){var b=TS.channels.getChannelById(a.channel);
if(!b){TS.error('unknown channel: "'+a.channel);
return
}TS.info("deleted channel "+a.channel);
TS.channels.removeChannel(b)
},channel_archive:function(a){var b=TS.channels.getChannelById(a.channel);
if(!b){TS.error('unknown channel: "'+a.channel);
return
...
b.is_archived=true;
if(!TS.model.user.is_restricted){if(b.is_member){b.was_archived_this_session=true
}}TS.channels.archived_sig.dispatch(b)
},channel_unarchive:function(a){var c=TS.channels.getChannelById(a.channel);
if(!c){TS.error('unknown channel: "'+a.channel);
return
}if(!c.is_archived){return
}TS.info("unarchived channel "+a.channel);
if(c.was_archived_this_session){var b=true;
TS.channels.join(c.name,null,b)
}c.is_archived=false;
c.was_archived_this_session=false;
TS.channels.unarchived_sig.dispatch(c)
},channel_rename:function(a){var b=TS.channels.getChannelById(a.channel.id);
if(!b){TS.error('unknown channel: "'+a.channel);
return
}TS.info("renamed channel "+a.channel.id+" to "+a.channel.name);
TS.channels.channelRenamed(a.channel)
},subtype__channel_join:function(b){var a=b.user;
var f=TS.members.getMemberById(a);
if(!f){TS.error('unknown member: "'+a+'"');
return
...
if(e){for(var d=0;
d<e.members.length;
d++){if(e.members[d]==f.id){c=e.members[d];
break
}}}if(!c){e.members.push(f.id);
TS.channels.calcActiveMembersForChannel(e)
}if(f.is_self&&b.inviter){e.needs_invited_message=true;
e.inviter=b.inviter
}TS.members.invalidateMembersUserCanSeeArrayCaches();
TS.channels.member_joined_sig.dispatch(e,f)
},channel_marked:function(a){var b=TS.channels.getChannelById(a.channel);
if(!b){TS.error('unknown channel: "'+a.channel+'"');
return
}b.needs_invited_message=false;
TS.channels.setLastRead(b,a.ts)
},subtype__channel_topic:function(b){var c=TS.channels.getChannelById(b.channel);
if(!c){TS.error('unknown channel: "'+b.channel+'"');
return
}var a=b.user;
var d=TS.members.getMemberById(a);
if(!d){TS.error('unknown member: "'+a+'"');
return
}TS.info(d.name+" changed topic for channel "+b.channel+" to "+b.topic);
TS.channels.topicChanged(c,a,b.ts,b.topic)
},subtype__channel_purpose:function(b){var c=TS.channels.getChannelById(b.channel);
if(!c){TS.error('unknown channel: "'+b.channel+'"');
return
}var a=b.user;
var d=TS.members.getMemberById(a);
...
},channel_history_changed:function(a){TS.ms.msg_handlers.channel_history_changed_worker(a)
},subtype__channel_history_changed:function(a){if(TS.boot_data.feature_channel_eventlog_client){TS.warn("feature_channel_eventlog_client=1 so we should never be getting subtype "+a.subtype)
}TS.ms.msg_handlers.channel_history_changed_worker(a)
},channel_history_changed_worker:function(a){var b=TS.channels.getChannelById(a.channel);
if(!b){TS.error('unknown channel: "'+a.channel+'"');
return
}if(!b.is_member){TS.warn("we can ignore this channel_history_changed msg, we are not a member");
return
}b.history_changed=true;
TS.channels.fetchHistory(b,{channel:b.id,latest:a.latest,count:TS.utility.clamp(b.msgs.length,TS.model.initial_msgs_cnt,1000)},function(e,f,c){if(!e){TS.error("could not retrieve history")
}else{b.oldest_msg_ts=null;
TS.storage.storeOldestTs(b.id,null);
TS.warn("imsg.latest: "+a.latest);
for(var d=b.msgs.length-1;
d>-1;
d--){var g=b.msgs[d];
if(g.ts<a.latest){continue
...
if(b.id==TS.model.active_channel_id){}else{}},group_left:function(a){TS.info("You left group "+a.channel);
var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel);
return
}TS.groups.removeGroup(b);
TS.members.invalidateMembersUserCanSeeArrayCaches();
TS.groups.left_sig.dispatch(b)
},subtype__group_leave:function(b){var a=b.user;
var e=TS.members.getMemberById(a);
if(!e){TS.error('unknown member: "'+a+'"');
return
...
},group_joined:function(a){TS.info("You joined group "+a.channel.name);
var b=TS.groups.getGroupById(a.channel.name);
if(b){TS.error("should not be getting a group_joined message if we already know about the group: "+a.channel.name+" "+a.channel.id);
return
}var c=TS.groups.upsertGroup(a.channel);
TS.members.invalidateMembersUserCanSeeArrayCaches();
TS.groups.joined_sig.dispatch(c);
TS.shared.checkInitialMsgHistory(c,TS.groups)
},group_deleted:function(a){var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel);
return
}TS.info("deleted group "+a.channel);
TS.groups.removeGroup(b)
},group_archive:function(a){var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel);
return
}if(b.is_archived){return
}TS.info("archived group "+a.channel);
b.is_archived=true;
if(b.is_open){b.was_archived_this_session=true
}TS.groups.archived_sig.dispatch(b)
},group_unarchive:function(a){var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel);
return
}if(!b.is_archived){return
...
},group_rename:function(a){var b=TS.groups.getGroupById(a.channel.id);
if(!b){TS.error('unknown group: "'+a.channel.id);
return
}TS.info("renamed group "+a.channel.id+" to "+a.channel.name);
TS.groups.groupRenamed(a.channel)
},subtype__group_join:function(b){var a=b.user;
var f=TS.members.getMemberById(a);
if(!f){TS.error('unknown member: "'+a+'"');
return
...
},group_open:function(a){var c=TS.groups.getGroupById(a.channel);
if(!c){TS.error("unkown group! "+a.channel);
return
}var b=c.is_open;
c.is_open=true;
if(TS.model.requested_group_opens[a.channel]){TS.groups.displayGroup(c.id,false,TS.model.requested_group_opens[a.channel].and_send_txt);
delete TS.model.requested_group_opens[a.channel]
}c.opened_this_session=true;
if(!b){TS.groups.opened_sig.dispatch(c);
TS.shared.checkInitialMsgHistory(c,TS.groups)
}},group_marked:function(a){var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel+'"');
return
}b.needs_invited_message=false;
TS.groups.setLastRead(b,a.ts)
},group_close:function(a){var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel+'"');
return
}b.is_open=false;
if(TS.model.active_group_id==a.channel){TS.client.activeChannelDisplayGoneAway()
}TS.groups.closed_sig.dispatch(b)
},subtype__group_topic:function(b){var c=TS.groups.getGroupById(b.channel);
...
},subtype__group_purpose:function(b){var c=TS.groups.getGroupById(b.channel);
if(!c){TS.error('unknown group: "'+b.channel+'"');
return
}var a=b.user;
var d=TS.members.getMemberById(a);
...
},group_history_changed:function(a){TS.ms.msg_handlers.group_history_changed_worker(a)
},subtype__group_history_changed:function(a){if(TS.boot_data.feature_channel_eventlog_client){TS.warn("feature_channel_eventlog_client=1 so we should never be getting subtype "+a.subtype)
}TS.ms.msg_handlers.group_history_changed_worker(a)
},group_history_changed_worker:function(a){var b=TS.groups.getGroupById(a.channel);
if(!b){TS.error('unknown group: "'+a.channel+'"');
return
}b.history_changed=true;
TS.groups.fetchHistory(b,{channel:b.id,latest:a.latest,count:TS.utility.clamp(b.msgs.length,TS.model.initial_msgs_cnt,1000)},function(e,f,c){if(!e){TS.error("could not retrieve history")
}else{b.oldest_msg_ts=null;
TS.storage.storeOldestTs(b.id,null);
TS.warn("imsg.latest: "+a.latest);
for(var d=b.msgs.length-1;
...
if(b.id==TS.model.active_group_id){}else{}},im_created:function(b){var a=TS.ims.getImById(b.channel.id);
if(a){TS.error("we already have an im for this user! "+b.user);
return
}TS.ims.upsertIm(b.channel);
a=TS.ims.getImById(b.channel.id);
if(!a){TS.error("WTF why can we not find this im: "+b.channel.id);
return
}TS.members.invalidateMembersUserCanSeeArrayCaches();
if(a.is_open){if(TS.model.requested_im_opens[b.user]){TS.ims.displayIm(a.id,false,TS.model.requested_im_opens[b.user].and_send_txt);
delete TS.model.requested_im_opens[b.user]
}TS.ims.opened_sig.dispatch(a)
}a.opened_this_session=true
},im_open:function(b){var a=TS.ims.getImById(b.channel);
if(!a){TS.error("unkown im! "+b.channel);
return
}var c=a.is_open;
a.is_open=true;
if(TS.model.requested_im_opens[b.user]){TS.ims.displayIm(a.id,false,TS.model.requested_im_opens[b.user].and_send_txt);
delete TS.model.requested_im_opens[b.user]
}a.opened_this_session=true;
if(!c){TS.ims.opened_sig.dispatch(a);
TS.shared.checkInitialMsgHistory(a,TS.ims)
}},im_marked:function(b){var a=TS.ims.getImById(b.channel);
if(!a){TS.error('unknown im: "'+b.channel+'"');
return
}TS.ims.setLastRead(a,b.ts)
},im_close:function(b){var a=TS.ims.getImById(b.channel);
if(!a){TS.error('unknown im: "'+b.channel+'"');
return
}a.is_open=false;
if(TS.model.active_im_id==b.channel){TS.client.activeChannelDisplayGoneAway()
}TS.ims.closed_sig.dispatch(a)
},im_history_changed:function(a){TS.ms.msg_handlers.im_history_changed_worker(a)
},subtype__im_history_changed:function(a){if(TS.boot_data.feature_channel_eventlog_client){TS.warn("feature_channel_eventlog_client=1 so we should never be getting subtype "+a.subtype)
}TS.ms.msg_handlers.im_history_changed_worker(a)
},im_history_changed_worker:function(b){var a=TS.ims.getImById(b.channel);
if(!a){TS.error('unknown im: "'+b.channel+'"');
return
}a.history_changed=true;
TS.ims.fetchHistory(a,{channel:a.id,latest:b.latest,count:TS.utility.clamp(a.msgs.length,TS.model.initial_msgs_cnt,1000)},function(e,f,c){if(!e){TS.error("could not retrieve history")
}else{a.oldest_msg_ts=null;
TS.storage.storeOldestTs(a.id,null);
TS.warn("imsg.latest: "+b.latest);
for(var d=a.msgs.length-1;
d>-1;
d--){var g=a.msgs[d];
if(g.ts<b.latest){continue
}TS.warn("unshifting: "+g.ts);
f.messages.unshift(g)
}a.msgs.length=0;
TS.ims.onHistory(e,f,c)
}delete a.history_changed
});
if(a.id==TS.model.active_im_id){}else{}},manual_presence_change:function(b){var a=TS.model.user;
...
},presence_change:function(a){var b=TS.members.getMemberById(a.user);
if(!b){TS.error('unknown member: "'+a.user+'"');
return
}if(a.presence!="away"&&a.presence!="active"){TS.error('unknown presence: "'+a.presence+'"');
return
}if(b.presence==a.presence){return
}b.presence=a.presence;
TS.members.presence_changed_sig.dispatch(b)
},status_change:function(a){var b=TS.members.getMemberById(a.user);
if(!b){TS.error('unknown member: "'+a.user+'"');
return
}if(b.status==a.status){return
}b.status=a.status;
TS.members.status_changed_sig.dispatch(b)
},pref_change:function(a){TS.prefs.onPrefChanged(a)
},team_pref_change:function(a){TS.prefs.onTeamPrefChanged(a)
},file_created:function(a){var b=TS.files.getFileById(a.file.id);
if(b){TS.warn("we already know about this file, which probably means the files.upload response came in before this message (so np) "+a.file.id)
}else{TS.files.upsertAndSignal(a.file)
}},file_public:function(a){TS.files.upsertAndSignal(a.file);
if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},file_deleted:function(a){TS.files.removeFile(a.file_id);
if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},file_private:function(a){TS.files.fetchFileInfo(a.file_id);
if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},file_change:function(a){TS.files.upsertAndSignal(a.file);
TS.files.fileWasMaybeRefreshed(a.file);
if(a.file.mode=="snippet"||a.file.mode=="post"){TS.files.fetchFileInfo(a.file.id)
}},file_shared:function(a){TS.files.upsertAndSignal(a.file)
},file_unshared:function(a){TS.files.upsertAndSignal(a.file)
},file_comment_added:function(a){var b=TS.files.getFileById(a.file.id);
if(!b){return
}TS.files.editCommentOnFile(a.comment,b);
TS.files.upsertFile(a.file);
if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},file_comment_edited:function(a){var b=TS.files.getFileById(a.file.id);
if(!b){return
}TS.files.editCommentOnFile(a.comment,b);
TS.files.upsertFile(a.file)
},file_comment_deleted:function(a){var b=TS.files.getFileById(a.file.id);
if(!b){return
}TS.files.deleteCommentOnFile(a.comment,b);
TS.files.upsertFile(a.file);
if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},hello:function(a){},team_join:function(a){var b=a.user;
TS.info(b.name+" joined the team");
TS.members.upsertMember(b);
b=TS.members.getMemberById(b.id);
if(!b){TS.error("wtf no member "+b.id+"?");
return
}TS.members.joined_team_sig.dispatch(b);
TS.view.showProperTeamPaneFiller()
},user_change:function(a){var b=TS.members.getMemberById(a.user.id);
if(!b){TS.error("wtf no member "+a.user.id+"?");
return
}TS.members.upsertAndSignal(a.user)
},star_added:function(a){if(!a.item){TS.error(a.type+" has no item");
return
}if(a.user==TS.model.user.id){TS.stars.starStatusHasChanged(true,a.item,a.type);
TS.stars.maybeUpdateStarredItems()
}else{TS.activity.slurpStarItem(a.item,a.type)
}if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},star_removed:function(a){if(!a.item){TS.error(a.type+" has no item");
return
}if(a.user==TS.model.user.id){TS.stars.starStatusHasChanged(false,a.item,a.type);
TS.stars.maybeUpdateStarredItems()
}else{TS.activity.slurpStarItem(a.item,a.type)
}if(a.event_ts){TS.activity.maybeUpdateTeamActivity()
}},email_domain_changed:function(a){TS.model.team.email_domain=a.email_domain;
TS.view.showProperTeamPaneFiller()
},team_domain_change:function(a){TS.model.last_team_domain=TS.model.team.domain=a.domain;
TSSSB.call("setCurrentTeam",TS.model.team.domain)
},slack_broadcast:function(b){var e=null;
var g=b.title||"Broadcast message";
var a=b.body||"";
var f="";
var d=b.button||(b.reload?"Reload":"OK");
var h=false;
if(!!b.reload){if(!!b.force_reload){TS.info("reloading because imsg.force_reload");
h=true
}else{if(!TS.boot_data.svn_rev){TS.info("reloading because we dont have an svn_rev");
h=true
}else{if(b.svn_rev=="dev"){TS.info("reloading because dev");
h=true
}else{if(parseInt(TS.boot_data.svn_rev)<parseInt(b.svn_rev)){TS.info("reloading because "+TS.boot_data.svn_rev+" < "+b.svn_rev);
h=true
}}}}if(!h){return
}e=function(){TS.client.reload()
}
}if(h){var c=TS.utility.randomInt(10,20);
f='<p class="top_margin">(You will be auto reloaded in <span id="auto_secs">'+c+"</span> seconds.)</p>";
setTimeout(function(){TS.client.reload()
},c*1000);
setInterval(function(){c--;
if(c<1){return
}$("#auto_secs").text(c)
},1000)
}TS.generic_dialog.start({title:TS.format.formatWithOptions(g,null,{no_highlights:true,no_specials:true}),body:TS.format.formatWithOptions(a,null,{no_highlights:true,no_specials:true})+f,go_button_text:d,show_cancel_button:false,esc_for_ok:true,on_go:e})
},team_rename:function(a){$("#team_name").text(a.name);
document.title=document.title.replace(TS.model.last_team_name,a.name);
if(TS.ui.growls.original_document_title){TS.ui.growls.original_document_title=TS.ui.growls.original_document_title.replace(TS.model.last_team_name,a.name)
}TS.model.last_team_name=TS.model.team.name=a.name
},team_icon_change:function(a){if(!a.icon){return
}TS.model.team.icon=a.icon;
if(TS.client){TS.client.updateTeamIcon()
}},bot_added:function(a){var b=a.bot;
TS.info(b.name+" was added");
TS.bots.upsertBot(b);
b=TS.bots.getBotById(b.id);
if(!b){TS.error("wtf no bot "+b.id+"?");
return
}TS.bots.added_sig.dispatch(b)
},bot_changed:function(a){var b=TS.bots.getBotById(a.bot.id);
if(!b){TS.error("wtf no bot "+a.bot.id+"?");
return
}TS.bots.upsertAndSignal(a.bot)
},bot_removed:function(a){var b=TS.bots.getBotById(a.bot.id);
if(!b){TS.error("wtf no bot "+a.bot.id+"?");
return
}TS.bots.upsertAndSignal(a.bot)
},error:function(a){},user_typing:function(c){var f=TS.members.getMemberById(c.user);
if(!f){TS.error("unknown imsg.user:"+c.user);
return
...
},issue_change:function(a){TS.help.onIssueChange(a.issue)
},emoji_changed:function(a){TS.setUpEmoji(function(){TS.prefs.setEmojiMode();
TS.model.emoji_names.length=0;
for(var b in emoji.data){var e=emoji.data[b][3];
for(var d=0;
d<e.length;
d++){var c=e[d];
TS.model.emoji_names.push(c)
}}TS.model.emoji_names.sort();
TS.client.ui.rebuildAll();
TS.client.ui.makeEmoticonList()
})
},play_sound:function(a){if(TS.model.prefs.autoplay_chat_sounds){TS.sounds.play(a.sound+".mp3")
}},commands_changed:function(a){TS.setUpCmds()
...
if(!!window.matchMedia){window.matchMedia("screen and (-webkit-min-device-pixel-ratio: 1.5), screen and (min--moz-device-pixel-ratio: 1.5), screen and (min-device-pixel-ratio: 1.5)").addListener(a)
...
return d
},load:function(){var b=TS.utility.date.getTimeStamp();
...
if(TS.boot_data.feature_spaces){TS.templates.spaces_connected_members=TS.templates.compile("spaces_connected_members");
TS.templates.spaces_connected_member=TS.templates.compile("spaces_connected_member")
}TS.info(TS.utility.date.getTimeStamp()-b+"ms spent compiling templates")
...
},fileHTML:function(b,c){var e=TS.members.getMemberById(b.user);
var a;
var d={member:e,file:b,for_search:c,icon_class:TS.utility.getImageIconClass(b,"thumb_80"),is_post:b.mode=="post",is_snippet:b.mode=="snippet",is_hosted_or_external:b.mode=="hosted"||b.mode=="external"||b.mode=="space"};
...
if(TS.utility.isRetina()&&G.file.thumb_720){n.show_retina_thumb=true
...
var h=[];
var b=[];
l.sort(function(m,i){return(m._name_lc>i._name_lc)?1:((i._name_lc>m._name_lc)?-1:0)
});
for(var e=0;
e<l.length;
e++){f=l[e];
...
return c
},makeMemberPreviewLinkImage:function(b,l,c,h){var d=TS.members.getMemberById(b);
if(!d||!d.profile){return""
}c=(c===true);
h=(h===true);
var i,k;
var j,a;
var f=TS.utility.isRetina();
j="background-image: ";
a=[];
switch(l){case 24:if(f){i=d.profile.image_48
}else{i=d.profile.image_24
}k="thumb_24";
break;
case 32:if(f){i=d.profile.image_72
}else{i=d.profile.image_32
}k="thumb_32";
break;
case 36:if(f){i=d.profile.image_72
}else{i=d.profile.image_48
}k="thumb_36";
break;
case 48:if(f){i=d.profile.image_72
}else{i=d.profile.image_48
}k="thumb_48";
break;
case 72:if(f){i=d.profile.image_192
}else{i=d.profile.image_72
}k="thumb_72";
break;
case 192:i=d.profile.image_192;
k="thumb_192";
break;
default:if(f){i=d.profile.image_72
}else{i=d.profile.image_48
}k="thumb_48";
break
}if(d.is_restricted){if(f){a.push("url('"+cdn_url+"/27603/img/avatar_overlays_@2x.png')")
}else{a.push("url('"+cdn_url+"/27603/img/avatar_overlays.png')")
}}if(d.is_ultra_restricted){k+=" ura"
}else{if(d.is_restricted){k+=" ra"
}else{if(TS.boot_data.feature_bot_users&&d.is_bot){k+=" bot"
}}}var e;
var g=(TS.client)?'target="/team/'+d.name+'"':"";
a.push("url('"+i+"')");
if(c){j=(a.length?a.join(", "):"");
if(h){e='<span class="lazy member_preview_link member_image '+k+'" data-member-id="'+d.id+'" data-thumb-size="'+l+'" data-original="'+j+'" style="background-color: #f6f6f6" aria-hidden="true"></span>'
}else{e='<a href="/team/'+d.name+'" '+g+' class="lazy member_preview_link member_image '+k+'" data-member-id="'+d.id+'" data-thumb-size="'+l+'" data-original="'+j+'" style="background-color: #f6f6f6" aria-hidden="true"></a>'
}}else{j=(a.length?j+a.join(", "):"");
if(h){e='<span class="member_preview_link member_image '+k+'" data-member-id="'+d.id+'" data-thumb-size="'+l+'" style="'+j+'" aria-hidden="true"></span>'
}else{e='<a href="/team/'+d.name+'" '+g+' class="member_preview_link member_image '+k+'" data-member-id="'+d.id+'" data-thumb-size="'+l+'" style="'+j+'" aria-hidden="true"></a>'
}}return e
...
if(g){b+='&nbsp;<a class="unshare_link" onclick="TS.files.promptForFileUnshare(\''+c.id+"', '"+d.id+'\')" data-toggle="tooltip" title="Remove this from #'+d.name+'"><i class="fa fa-minus-circle"></i></a>'
}b+="</span>";
e.push(b)
}var f;
for(a=0;
...
if(g){b+='&nbsp;<a class="unshare_link" onclick="TS.files.promptForFileUnshare(\''+c.id+"', '"+f.id+'\')" data-toggle="tooltip" title="Remove this from the '+f.name+' group"><i class="fa fa-minus-circle"></i></a>'
}b+="</span>";
e.push(b)
}if(!e.length){return""
}return e.join(", ")
...
if(TS.model.active_channel_id==b.id){c="channel"
}else{if(TS.model.active_im_id==b.id){c="im"
}else{if(TS.model.active_group_id==b.id){c="group"
}else{return""
}}}var m=[];
var o;
...
Handlebars.registerHelper("isWeb",function(e){if(TS.boot_data.app=="web"||TS.boot_data.app=="mobile"){return e.fn(this)
}else{return e.inverse(this)
}});
...
Handlebars.registerHelper("makeUsernameImage",function(g,p){var m,f,l,e;
var h;
var o=(g.bot_id)?TS.bots.getBotById(g.bot_id):null;
var j=TS.utility.isRetina();
if(g.icons){h=g.icons
}else{if(o&&o.icons){h=o.icons
}else{}}if(h){if(h.image_36&&!j){m=h.image_36
}else{if(h.image_72&&j){m=h.image_72
}else{if(h.image_48){m=h.image_48
}else{if(h.emoji&&h.emoji.substr(0,1)==":"&&h.emoji.substr(h.emoji.length-1,1)==":"){l=h.emoji
}}}}}var k=TS.templates.builders.makeBotLink(o,g.username);
var n=(g&&g.is_ephemeral&&g.username=="slackbot")?TS.members.getMemberById("USLACKBOT"):null;
switch(p){case 24:f="thumb_24";
e="https://i0.wp.com/slack-assets2.s3-us-west-2.amazonaws.com/8390/img/avatars/ava_0002-24.png?ssl=1";
if(n){e=n.profile.image_24
}break;
case 32:f="thumb_32";
e="https://i0.wp.com/slack-assets2.s3-us-west-2.amazonaws.com/8390/img/avatars/ava_0002-32.png?ssl=1";
if(n){e=n.profile.image_32
}break;
case 36:f="thumb_36";
e="https://i0.wp.com/slack-assets2.s3-us-west-2.amazonaws.com/8390/img/avatars/ava_0002-48.png?ssl=1";
if(n){e=n.profile.image_48
}break;
case 72:f="thumb_72";
e="https://i0.wp.com/slack-assets2.s3-us-west-2.amazonaws.com/8390/img/avatars/ava_0002-72.png?ssl=1";
if(n){e=n.profile.image_72
}break;
case 192:f="thumb_192";
e="https://i0.wp.com/slack-assets2.s3-us-west-2.amazonaws.com/8390/img/avatars/ava_0002-192.png?ssl=1";
if(n){e=n.profile.image_192
}break;
default:f="thumb_48";
e="https://i0.wp.com/slack-assets2.s3-us-west-2.amazonaws.com/8390/img/avatars/ava_0002-48.png?ssl=1";
if(n){e=n.profile.image_48
...
if(m){i=k.start_a+'<img style="border: 0" src="'+m+'" class="member_image '+f+'" />'+k.end_a
}else{if(l){i=k.start_a+'<div style="border: 0" class="member_image '+f+'">'+TS.utility.emojiGraphicReplace(TS.utility.htmlEntities(l),true,false,true)+"</div>"+k.end_a
}else{if(n){i=k.start_a+'<img src="'+e+'" class="member_image '+f+'" />'+k.end_a
}else{i=k.start_a+'<img src="'+e+'" class="member_image bot_icon_default '+f+'" />'+k.end_a
}}}return i
});
...
var f=(TS.client)?'target="/messages/@'+e.name+'"':"";
...
function a(i){var k=TS.utility.htmlEntities(i.username);
var h;
var j=(i.bot_id)?TS.bots.getBotById(i.bot_id):null;
var f=TS.utility.isRetina();
if(i.icons){h=i.icons
}else{if(j&&j.icons){h=j.icons
}else{}}if(!k&&j&&j.name){k=TS.utility.htmlEntities(j.name)
}if(TS.members.botNameMatchesMemberName(k)){k+=" (bot)"
}var e=TS.templates.builders.makeBotLink(j,i.username);
if(!h){return e.start_a+k+e.end_a
}var g;
if(h.emoji&&h.emoji.substr(0,1)==":"&&h.emoji.substr(h.emoji.length-1,1)==":"){g=e.start_a+TS.utility.emojiGraphicReplace(TS.utility.htmlEntities(h.emoji),true,false,true)+e.end_a+" "+e.start_a+k+e.end_a
}else{if(h.image_36&&!f){g=e.start_a+'<img src="'+h.image_36+'" class="inline_bot_icon">'+e.end_a+" "+e.start_a+k+e.end_a
}else{if(h.image_72&&f){g=e.start_a+'<img src="'+h.image_72+'" class="inline_bot_icon">'+e.end_a+" "+e.start_a+k+e.end_a
}else{if(h.image_48){g=e.start_a+'<img src="'+h.image_48+'" class="inline_bot_icon">'+e.end_a+" "+e.start_a+k+e.end_a
}else{g=e.start_a+k+e.end_a
}}}}return g
...
Handlebars.registerHelper("makeFileHeader",function(e,f){return TS.templates.file_header({file:e,member:f})
});
Handlebars.registerHelper("makeFilePreviewHeader",function(e,f){return TS.templates.file_header({file:e,member:f,preview:true})
});
...
(function(){TS.registerModule("utility.date",{month_names:["January","February","March","April","May","June","July","August","September","October","November","December"],short_month_names:["Jan","Feb","March","April","May","June","July","Aug","Sept","Oct","Nov","Dec"],day_names:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],short_day_names:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],onStart:function(){},toDateObject:function(d){var b;
if(d&&typeof d=="string"&&d.indexOf("-")>-1){var a=d.split("-");
if(a.length>=3){b=new Date(a[0],a[1]-1,a[2])
}else{b=new Date(0)
}}else{var c=(d||"0").toString();
if(c.indexOf(".")!=-1){b=new Date(c.split(".")[0]*1000)
}else{b=new Date(d*1000)
}}return b
},toTime:function(e,g,j){var a=TS.utility.date.toDateObject(e);
var h=a.getHours();
var c=a.getMinutes();
var d=a.getSeconds();
var b=false;
if(TS.utility.date.do24hrTime()){if(h<10){h="0"+h
...
if(g&&!TS.utility.date.do24hrTime()){if(b){i+=" PM"
}else{i+=" AM"
}}return i
},toDate:function(e){var a=TS.utility.date.toDateObject(e);
var f=a.getFullYear();
var d=a.getMonth();
var g=a.getDate();
var h=a.getHours();
...
var i=f+"-"+d+"-"+g+", "+h+":"+c;
if(!TS.utility.date.do24hrTime()){if(b){i+=" PM"
}else{i+=" AM"
}}return i
},toCalendarDateOrNamedDayShort:function(d){var e=false;
var c=TS.utility.date.toDateObject(d);
var b=new Date();
var a=31*24*60*60*1000;
if(c.getFullYear()==b.getFullYear()||b-c<=a){e=true
...
},toFilenameFriendlyDate:function(e){var a=TS.utility.date.toDateObject(e);
var f=a.getFullYear();
var d=a.getMonth();
var g=a.getDate();
var h=a.getHours();
var c=a.getMinutes();
var b=false;
if(!TS.utility.date.do24hrTime()){if(h>=12){if(h>12){h=h-12
}b=true
}else{if(h===0){h=12
}}}if(g<10){g="0"+g
}if(h<10){h="0"+h
...
var i=f+"_"+d+"_"+g+" "+h+"_"+c;
if(!TS.utility.date.do24hrTime()){if(b){i+=" PM"
}else{i+=" AM"
}}return i
},toCalendarDate:function(e,c,g){var b=TS.utility.date.toDateObject(e);
var d=b.getFullYear();
var f=b.getMonth();
var a=b.getDate();
var h;
if(c){h=TS.utility.date.short_month_names[f]+" "+TS.utility.ordinalNumber(a)
}else{h=TS.utility.date.month_names[f]+" "+TS.utility.ordinalNumber(a)
}if(!g){h+=", "+d
}return h
},toCalendarDateOrNamedDay:function(e,c,f){var b=TS.utility.date.toDateObject(e);
var a=new Date();
var d=new Date();
d.setDate(a.getDate()-1);
var g;
if(TS.utility.date.sameDay(b,a)){g="Today "
}else{if(TS.utility.date.sameDay(b,d)){g="Yesterday "
}else{g=TS.utility.date.toCalendarDate(e,c,f)
}}return g
},toCalendarDateIfYesterdayOrToday:function(e,c){var b=TS.utility.date.toDateObject(e);
var a=new Date();
var d=new Date();
d.setDate(a.getDate()-1);
var f="";
if(TS.utility.date.sameDay(b,a)){f=TS.utility.date.toCalendarDate(e,c)
}else{if(TS.utility.date.sameDay(b,d)){f=TS.utility.date.toCalendarDate(e,c)
}}return f
},toHour:function(d){var b=TS.utility.date.toDateObject(d);
var a=b.getHours();
var c=false;
if(TS.utility.date.do24hrTime()){if(a<10){a="0"+a
}}else{if(a>=12){if(a>12){a=a-12
}c=true
}else{if(a===0){a=12
}}}var e=a;
if(!TS.utility.date.do24hrTime()){if(c){e+=" PM"
}else{e+=" AM"
}}return""+e
},timezoneLabel:function(f,h){var a="Pacific Standard Time";
var b=-28800;
if(typeof f.tz_label!="undefined"){a=f.tz_label
}if(typeof f.tz_offset!="undefined"){b=f.tz_offset
}var g="<span title='"+a+"'><i class='fa fa-clock-o'></i> ";
if(f.id==TS.model.user.id){if(TS.client){g+=a+" (<a href='/account/settings' target='new'>change</a>)"
}else{g+=a+" (<a href='/account/settings'>change</a>)"
}}else{var d=b/60/60;
var j=(TS.model.user.tz_offset-b)/60/60;
if(h){var e=new Date();
var c=e.getTime();
var i=c-(j*60*60*1000);
g+='<span class="timezone_value">'+TS.utility.date.toTime(i/1000,true)+"</span>";
g+=" local time / "
}if(j===0){g+="in your timezone"
}else{g+=Math.abs(j)+" hour";
if(Math.abs(j)!=1){g+="s"
}}if(j>0){g+=" behind you"
}else{if(j<0){g+=" ahead of you"
}}if(d===0){g+=" (GMT)"
}else{if(d<0){g+=" (GMT"+d+")"
}else{if(d>0){g+=" (GMT+"+d+")"
}}}}g+="</span>";
...
},fake_ts_unique_incrementer:"0",fake_ts_unique_padder:"x",makeTsStamp:function(e,b,c){e=e||new Date().getTime();
...
return a+"."+f
},sameDay:function(a,b){return((a.getFullYear()==b.getFullYear())&&(a.getMonth()==b.getMonth())&&(a.getDate()==b.getDate()))
},sameHour:function(a,b){return((a.getFullYear()==b.getFullYear())&&(a.getMonth()==b.getMonth())&&(a.getDate()==b.getDate())&&(a.getHours()==b.getHours()))
},distanceInMinutes:function(a,d){var c=Math.round(a.getTime()/1000)-Math.round(d.getTime()/1000);
var b=c/60;
return b
},getNextActivityDayStamp:function(d){var c=TS.utility.date.toDateObject(d);
...
return a
},getPrevActivityDayStamp:function(d){var c=TS.utility.date.toDateObject(d);
var b=new Date(c.getTime()-86400000);
var a=b.getFullYear()+"-"+TS.utility.padNumber(b.getMonth()+1,2,"0")+"-"+TS.utility.padNumber(b.getDate(),2,"0");
return a
}})
...
return a
},removeFileSharesAndMentions:function(b,d){var e=b.msgs;
var a;
...
}}}}},removeFileComments:function(b,d){var e=b.msgs;
var a;
for(var c=e.length-1;
c>-1;
c--){a=e[c];
...
}}}}},removeFileReferences:function(b,e){var f=b.msgs;
var a;
...
(function(){TS.registerModule("utility",{keymap:{alt:18,ctrl:17,cmd_ff:224,cmd_other:91,cmd_right:93,esc:27,shift:16,tab:9,del:8,enter:13,left:37,up:38,right:39,down:40,pageup:33,pagedown:34,end:35,home:36,space:32,semicolon:59,comma:188,period:190,left_square_bracket:219,right_square_bracket:221,V:86,insert:45},keymap_reverse:{"18":"alt","17":"ctrl","224":"cmd_ff","91":"cmd_other","93":"cmd_right","27":"esc","16":"shift","9":"tab","8":"del","13":"enter","37":"left","38":"up","39":"right","40":"down","188":"comma","190":"period","219":"left_square_bracket","221":"right_square_bracket","86":"V","45":"insert"},onStart:function(){},isRetina:function(c){c=c||window;
...
var d=h.toLowerCase();
var e;
if(d=="@everyone"||d.substr(0,10)=="@everyone."||d.substr(0,10)=="@everyone-"||d.substr(0,10)=="@everyone_"){e="<!everyone>";
g=d.substr(9)
}else{if(d=="@channel"||d.substr(0,9)=="@channel."||d.substr(0,9)=="@channel-"||d.substr(0,9)=="@channel_"){e="<!channel>";
g=d.substr(8)
}else{if(d=="@group"||d.substr(0,7)=="@group."||d.substr(0,7)=="@group-"||d.substr(0,7)=="@group_"){e="<!group>";
g=d.substr(6)
}}}if(e){return i+e+g
...
if(h&&h.substr(h.length-1,1)==j){var k=h.substr(0,h.length-1);
g=j;
...
if(f&&f.substr(f.length-1,1)==h){var i=f.substr(0,f.length-1);
e=h;
...
},formatWithOptions:function(m,h,l){var f=(l&&!!l.do_inline_imgs)||undefined;
var r=(l&&!!l.for_growl)||undefined;
var j=(l&&!!l.for_edit)||undefined;
var c=(l&&!!l.no_highlights)||undefined;
var t=(l&&!!l.no_specials)||undefined;
var q=(l&&!!l.enable_slack_action_links)||undefined;
c=(h&&("no_highlights" in h))?!!h.no_highlights:!!c;
if(t===true||t===false){t=t
}else{if(h&&("mrkdwn" in h)){t=(h.mrkdwn===false)
}else{t=false
}}if(j){t=true
}var n=(h&&h.no_emoji);
var p=!(f&&(!h||h.subtype!="bot_message"));
if(!m){return""
}var s=$.trim(m);
if(!s){return""
}var d=[];
...
}if(I){if(r||j){return TS.format.tokenizeStr(d,"#"+I.name)
}K=(TS.client)?'target="/archives/'+I.name+'"':"";
if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(d,TS.format.generic_link_open+"#"+I.name+TS.format.link_close)
}return TS.format.tokenizeStr(d,'<a href="/archives/'+I.name+'" '+K+' data-channel-name="'+I.name+'" data-channel-id="'+I.id+'" class="internal_channel_link">#'+I.name+TS.format.link_close)
}else{if(C.length>1&&C[1]){return TS.format.tokenizeStr(d,"#"+C[1])
}else{if(TS.model.user.is_restricted){return TS.format.tokenizeStr(d,"#unknown-channel")
}else{return TS.format.tokenizeStr(d,"#deleted-channel")
}}}}if(u.substr(0,1)=="@"){F=TS.utility.msgs.getMemberFromMemberMarkup(u);
if(F){if(r||j){return TS.format.tokenizeStr(d,"@"+F.name)
}K=(TS.client)?'target="/team/'+F.name+'" ':"";
var L=(c)?"@"+F.name:TS.format.doHighlighting("@"+F.name);
if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(d,TS.format.generic_link_open+L+TS.format.link_close)
}return TS.format.tokenizeStr(d,'<a href="/team/'+F.name+'" '+K+'data-member-name="'+F.name+'" class="internal_member_link">'+L+TS.format.link_close)
}else{return TS.format.tokenizeStr(d,u)
}}if(u.substr(0,1)=="!"){var G=u.substr(1);
if(G){G=G.split("|")[0]
}if(G=="everyone"){if(r||j){return TS.format.tokenizeStr(d,"@everyone")
}return TS.format.tokenizeStr(d,'<b class="mention_everyone">@everyone</b>')
}else{if(G=="channel"){if(r||j){return TS.format.tokenizeStr(d,"@channel")
}return TS.format.tokenizeStr(d,'<b class="mention_channel">@channel</b>')
}else{if(G=="group"){if(r||j){return TS.format.tokenizeStr(d,"@group")
}return TS.format.tokenizeStr(d,'<b class="mention_group">@group</b>')
}}}return TS.format.tokenizeStr(d,u)
}var J=u.split("|");
var v=J.shift();
v=v.replace(/\"/g,"&quot;");
var y=J.join("|")||v;
y=$.trim(y);
if(v.indexOf("<")===0){return TS.format.tokenizeStr(d,"&lt;"+u.replace(/</g,"&lt;").replace(/>/g,"&gt;")+"&gt;")
}y=y.replace(/</g,"&lt;");
y=y.replace(/>/g,"&gt;");
...
}}if(!t&&y!=v){y=TS.format.doSpecials(y,h&&h._special_debug)
}if(!n&&y!=v){y=TS.utility.emojiReplace(y,e)
}if(!c){y=TS.format.doHighlighting(y)
}var x;
if(v.indexOf(TS.utility.msgs.api_url_prefix+"chat.help")===0){if(q){x=TS.utility.htmlEntities(TS.utility.jsString(v));
return TS.format.tokenizeStr(d,'<a onclick="TS.utility.msgs.doApiUrl('+x+')" class="api_url">'+y+TS.format.link_close)
}else{return TS.format.tokenizeStr(d,'<a class="api_url muted">'+y+" (Disabled)"+TS.format.link_close)
}}else{if(v.indexOf(TS.utility.msgs.new_api_url_prefix)===0){if(q){x=TS.utility.htmlEntities(TS.utility.jsString(v));
return TS.format.tokenizeStr(d,'<a onclick="TS.utility.msgs.doNewApiUrl('+x+')" class="api_url">'+y+TS.format.link_close)
}else{return TS.format.tokenizeStr(d,'<a class="api_url muted">'+y+" (Disabled)"+TS.format.link_close)
}}else{if(v.indexOf("javascript:")==0){return TS.format.tokenizeStr(d,'<a onclick="'+v.replace("javascript:","")+'">'+y+TS.format.link_close)
}else{if(TS.client&&TS.client.core_url&&v.indexOf(TS.client.core_url)===0){if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(d,TS.format.generic_link_open+y+TS.format.link_close)
}return TS.format.tokenizeStr(d,'<a target="_self" href="'+v+'">'+y+TS.format.link_close)
}else{var H="";
var A;
if(h&&h.ts&&f){A=TS.inline_attachments.getAttachmentByFromUrl(h.attachments,v);
if(A){if(TS.boot_data.feature_attachments_inline||TS.templates.builders.shouldDoSimpleAttachment(A,h)){H=TS.templates.builders.buildAttachmentHTML({attachment:A,url:v,msg:h,show_initial_caret:true}).replace(/\n/g,"").replace(/\t/g,"").replace(/  /g," ")
}}}if(H){H=H.replace(/\n/g,"").replace(/\t/g,"").replace(/ +/g," ")
}if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(d,TS.format.generic_link_open+y+TS.format.link_close+H)
}return TS.format.tokenizeStr(d,"<a "+TS.utility.makeRefererSafeLink(v)+' target="_blank">'+y+TS.format.link_close+H)
}}}}};
s=s.replace(/<(.*?)>/g,o);
s=s.replace(/\</g,"&lt;");
s=s.replace(/\>/g,"&gt;");
if(r||j){s=s.replace(/\&lt\;/g,"<");
s=s.replace(/\&gt\;/g,">");
s=s.replace(/\&amp\;/g,"&")
}else{TS.format.special_token_map=[];
if(!t){s=TS.format.doSpecials(s,h&&h._special_debug)
}if(!p){s=s.replace(TS.format.hex_rx,TS.format.hexReplace)
}if(!n){s=TS.utility.emojiReplace(s,e)
}s=TS.format.deTokenizeStr(TS.format.special_token_map,s);
TS.format.special_token_map=null;
if(!c){s=TS.format.doHighlighting(s)
}s=s.replace(/<\/div>\n/g,"</div>");
s=s.replace(/<\/pre>\n/g,"</pre>");
...
s=s.replace(/  /g," &nbsp;");
s=s.replace(/^ /g,"&nbsp;")
}s=TS.format.deTokenizeStr(d,s);
s=TS.format.deTokenizeStr(e,s);
e=null;
if(window.TSF){try{var k="NORMAL";
if(t){k="NOMRKDWN"
}if(r){k="GROWL"
}if(j){k="EDIT"
}var i=a(m,h,k,t,c,n,p,f,q);
if(i!=s){TS.error("========================================================================"+(i==s));
TS.info("txt     :"+m);
TS.info("new_txt :"+s);
TS.info("tsf_str :"+i)
}else{if(false){TS.warn("================================"+(i==s));
TS.info("new_txt:"+s);
TS.info("tsf_str:"+i)
}}}catch(g){TS.error("error testing TSF:"+g);
TS.info("txt was:"+m)
}}return s
...
}}}}}}}}else{if(C.indexOf("<")==-1){v+=C.replace(/\&lt\;/g,"<").replace(/\&gt\;/g,">").replace(/\&amp\;/g,"&")
...
v+='<b class="mention_'+e+'">@'+e+"</b>"
}else{if(C.indexOf("<@")===0){e=C.replace(/<|>/g,"");
u=TS.utility.msgs.getMemberFromMemberMarkup(e);
if(u){F=(TS.client)?'target="/team/'+u.name+'" ':"";
...
s=o[0];
D=TS.channels.getChannelById(s);
if(D){F=(TS.client)?'target="/archives/'+D.name+'"':"";
...
TS.menu.$menu_footer.html(TS.templates.menu_channel_footer({channel:g,user:TS.model.user,show_topic:!TS.model.user.is_restricted&&(!g.is_general||TS.members.canUserPostInGeneral())}));
...
}else{if(d=="space"&&TS.boot_data.feature_spaces){return true
...
},startWithMemberPreview:function(g,d){if(TS.menu.isRedundantClick(g)){return
}if(TS.client&&TS.client.ui.checkForEditing(g)){return
}if(TS.model.menu_is_showing){return
}TS.menu.clean();
var f=$(g.target).closest(".member_preview_menu_target");
if(!d){d=f.closest("[data-member-id]").data("member-id")
}var i=TS.menu.member=TS.members.getMemberById(d);
var h={profile:true,member:i};
TS.menu.$menu_header.addClass("hidden").empty();
if(!i.deleted&&!i.is_slackbot&&d!=TS.model.user.id){if(!TS.model.user.is_ultra_restricted&&!i.is_ultra_restricted){var c=TS.members.getMyChannelsThatThisMemberIsNotIn(d);
if(c.length){h.show_channel_invite=true
}h.show_group_create=true;
if(TS.model.allow_invite_to_group_from_person){h.show_group_invite=true
}}}TS.menu.$menu_items.html(TS.templates.menu_member_items(h));
TS.menu.start(g);
TS.menu.$menu_items.find("li").bind("click.menu",TS.menu.onMemberItemClick);
TS.kb_nav.setSubmitItemHandler(TS.menu.onMemberItemClick);
if($(window).width()-f.offset().left<300){TS.menu.positionAt(f,-30,f.outerHeight()+5)
}else{TS.menu.positionAt(f,0,f.outerHeight()+5)
}TS.menu.keepInBounds()
...
TS.view.fileSetButtonState($(this).data("filetype"));
TS.menu.end()
},startWithFileMemberFilter:function(m,d){var h,g,f,c,l,k;
...
f=TS.members.getMembersForUser();
c=[];
l=[];
if(TS.boot_data.feature_bot_users){for(h=0,g=f.length;
h<g;
h++){if(f[h].is_bot||f[h].is_slackbot){l.push(f[h])
}else{c.push(f[h])
}}f=c.concat(l)
}TS.menu.$menu_items.html(TS.templates.menu_file_member_filter_items({members:f}));
if(d){TS.menu.$menu_items.find("li").bind("click.menu",TS.menu.onSearchFileMemberFilterItemClick)
}else{TS.menu.$menu_items.find("li").bind("click.menu",TS.menu.onFileMemberFilterItemClick)
}TS.menu.start(m);
if(d){TS.menu.positionAt($("#search_results_container"),102,100)
...
},startWithSpace:function(g){if(TS.menu.isRedundantClick(g)){return
}if(TS.model.menu_is_showing){return
}TS.menu.clean();
var h={file:TS.files.getFileById(TS.web.file.file_id),actions:{share:true,open_original:true,create_public_link:true,delete_file:true,print:true}};
TS.menu.$menu_header.addClass("hidden").empty();
TS.menu.$menu_items.html(TS.templates.menu_space_action_items(h));
...
},startWithServiceActions:function(d){if(TS.client&&!TS.model.ms_connected){TS.sounds.play("beep");
return
}if(TS.menu.isRedundantClick(d)){return
}if(TS.model.menu_is_showing){return
}TS.menu.clean();
TS.menu.$menu.addClass("headless footless");
var c=$(d.target).closest(".action_cog");
TS.menu.$menu_header.addClass("hidden").empty();
TS.menu.$menu_items.html(TS.templates.menu_service_action_items({can_enable:c.data("can_enable"),can_disable:c.data("can_disable"),can_remove:c.data("can_remove"),service_id:c.data("service_id")}));
TS.menu.$menu.addClass("no_min_width");
TS.menu.$menu_items.find("li").bind("click.menu",TS.menu.onServiceActionClick);
TS.menu.start(d)
},onServiceActionClick:function(c){c.preventDefault();
switch($(this).attr("id")){case"disable_service":if(confirm("Are you sure you want to disable this service integration?")){$("#services_action_disable").submit()
}break;
case"enable_service":if(confirm("Are you sure you want to re-enable this service integration?")){$("#services_action_enable").submit()
}break;
case"remove_service":if(confirm("Are you sure you want to remove this service integration?")){$("#services_action_remove").submit()
}break
}TS.menu.end()
...
if(g=="team_home"||g=="team_admin"||g=="team_services"||g=="team_invitations"||g=="team_apps"){}else{if(g=="team_help"){f.preventDefault();
TS.help_dialog.start()
}else{if(g=="logout"){f.preventDefault();
TS.utility.loadUrlInWindowIfOnline(TS.boot_data.logout_url)
}else{if(g=="add_team"){if(TSSSB.call("signInTeam")){f.preventDefault()
}else{$("body").addClass("hidden")
...
if(TS.boot_data.special_flex_panes){var c=TS.menu.$menu.width()-41;
...
}},"/away":{type:"client",autocomplete:true,alias_of:null,aliases:null,desc:'Toggle your "away" status',func:function(b,a,d,c){TS.members.toggleUserPresence();
TS.members.presence_changed_sig.add(function(e){if(!e||e.id!=TS.model.user.id){return
...
}},"/sleep":{type:"client",autocomplete:false,alias_of:null,aliases:null,desc:"",func:function(b,a,d,c){TS.client.sleep()
}},"/wake":{type:"client",autocomplete:false,alias_of:null,aliases:null,desc:"",func:function(b,a,d,c){TS.client.wake()
...
var h=g.getBounds();
TS.info("bounds: "+h);
var i=function(m,l){if(l==google.maps.GeocoderStatus.OK){new google.maps.Marker({map:g,position:m[0].geometry.location})
}else{TS.warn("Geocoder failed due to: "+l);
var k=new google.maps.places.PlacesService(g);
var j=function(o,n){if(n==google.maps.places.PlacesServiceStatus.OK){new google.maps.Marker({map:g,position:o[0].geometry.location})
}else{TS.warn("PlacesService failed due to: "+n)
}};
k.nearbySearch({bounds:h,keyword:c.query},j)
}};
f.geocode({address:c.query,bounds:h},i)
};
...
if(TS.boot_data.feature_comment_mentions_autocomplete){a.tab_complete_ui({id:"comment_input_tab_ui",min_width:300,narrow:!!TS.client,no_model_ob:true})
...
h.tab_complete_ui({id:"msg_edit_tab_ui"})
...
$("body").bind("keydown.close_message_edit_form",function(l){if(l.which==TS.utility.keymap.esc){if(!TS.model.menu_is_showing&&!TS.model.dialog_is_showing){setTimeout(TS.msg_edit.onCancelEdit,0)
}}});
...
if(TS.client&&a){TS.client.ui.instaScrollMsgsToBottom(false)
}$("#edit_controls").scrollintoview({duration:500,px_offset:-50});
h.focus();
...
if(e.show_cancel_button){d.find(".dialog_cancel").removeClass("hidden");
d.find(".close").removeClass("hidden")
}else{d.find(".dialog_cancel").addClass("hidden");
...
},showIssueScreen:function(a,d){var c=TS.help_dialog.getElsForScreen(TS.help_dialog.last_issue_screen);
var b=TS.help_dialog.getElsForScreen(a);
c.transition({opacity:0},100,function(){if(arguments.callee.run){return
}arguments.callee.run=true;
if(!TS.help_dialog.showing){return
}c.addClass("hidden");
b.removeClass("hidden").css("opacity",0).transition({opacity:1},100,function(){if(arguments.callee.run){return
}arguments.callee.run=true;
if(!TS.help_dialog.showing){return
}if(d){d()
}TS.client.ui.updateClosestMonkeyScroller($("#help_issues_scroller"))
